...
(function() {
	...
	VIZI.Renderer.prototype.createRenderer = function() {
		var normal_renderer = new THREE.WebGLRenderer({
			antialias: false
		});

		normal_renderer.setSize( window.innerWidth, window.innerHeight );
		// renderer.setClearColor( this.scene.fog.color, 1 );

		// Gamma settings make things look 'nicer' for some reason
		normal_renderer.gammaInput = true;
		normal_renderer.gammaOutput = true;

		normal_renderer.physicallyBasedShading = true;

		normal_renderer.shadowMapEnabled = true;
		normal_renderer.shadowMapSoft = true;

		this.domContainer.appendChild(normal_renderer.domElement);


		var renderer = new THREE.StereoEffect( normal_renderer, {worldScale: 100} );
